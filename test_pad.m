...
size_pad=4096;
data_rep=zeros(1,size_pad,2,n_sample);
n_pad=ceil(size_pad-size_sample)/2;

for i=1:n_sample
    cur_real=data(1,:,1,i);
    cur_imag=data(1,:,2,i);
    cur_real=padarray(cur_real,[0 n_pad],0,'both');
    cur_imag=padarray(cur_imag,[0 n_pad],0,'both');
    cur_real=cur_real(1:size_pad);
    cur_imag=cur_imag(1:size_pad);
